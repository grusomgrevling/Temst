...
### Overordnet sett gjøres følgende operasjoner i denne arbeidsflyten:
...
# Verdikategorisering av "inngripsfrie naturområder" og sammenslåing med andre kartlag
# Sammenslåing av naturvernområder (gjeldende og foreslåtte) med vernet natur 
...
#🟩# Hvordan brukes kartlaget "Inngrepsfrie naturområder": 
# Delene av kartlaget "Inngrepsfrie naturområder" flyttes kartlag for 
# verdikategorisert naturmangfold. "Villmarkspregede områder" kategoriseres som 
# natur av svært stor verdi. "Inngrepsfri sone 1" kategoriseres som natur av 
# stor verdi. "Inngrepsfri sone 2" kategoriseres som natur av middels verdi. 
...
# Administrative enheter: "a0000000c — kommune" = "Min kommune"
# Eksisterende naturvernområder: "a0000000b — Naturvernområde" = "Naturvernområder"
# Foreslått naturvernområder: "a00000009 — ForeslattVern_xxxx_kommune" = "Foreslått naturvernområde"
# Inngrepsfrie naturområder: "statusPolygon" = "Inngrepsfri natur"
...
# Eksakte navn til nytt navn
rename_map = {
    "a0000000a — artnasjonal_punkt": "Arter_punkter",
    "a00000009 — artnasjonal_omr": "Arter_flater",
    "a0000000b — Naturvernområde": "Naturvernområde",
    "statusPolygon": "Inngrepsfri natur",
    "a0000000c — kommune": "Min kommune"
}

# Prefix-baserte navn
NATUR_PREFIX = "a00000009 — Naturtyper_ku_verdi"
FORESLATT_VERN_PREFIX = "a00000009 — ForeslattVern"
...
foreslatt_vern_layer = None
kulturlandskap_layer = None
...
for name, layer in found_layers.items():
    ...
    elif name.startswith(FORESLATT_VERN_PREFIX):
        foreslatt_vern_layer = layer
...
if not naturtyper_layer:
    print('Kartlaget for naturtyper ble ikke funnet 😿')

if not foreslatt_vern_layer:
    print('Kartlaget for foreslått vern ble ikke funnet 😿')
...
if foreslatt_vern_layer:
    old_name = foreslatt_vern_layer.name()
    foreslatt_vern_layer.setName("Foreslått naturvernområde")
    print(f'Endret: "{old_name}" → "Foreslått naturvernområde"')
...
# 6: Fjerner naturtyper og arter utenfor kommunen
# 7: Fjerner overlapp med fulldyrket mark (AR5)
...
#🟩# 6: Fikser geometriene og fjerner areal utenfor kommunen
...
kommune = project.mapLayersByName("Min kommune")[0]
...
#Fix geometry 
    ...
    if name != "Arter og naturtyper av potensiell verdi":
...
# Fix geometry
    ...
    # Erstatt gammelt lag
    project.removeMapLayer(original_layer.id())
    project.addMapLayer(layer)

print("Alle lag er nå begrenset til Min kommune.")
...
ar5 = project.mapLayersByName("a0000000b — fkb_ar5_omrade")[0]

# Fjerner overlapp med fulldyrka mark (arealtype 21)

fulldyrka = processing.run(
    "native:extractbyexpression",
    {
        "INPUT": ar5,
        "EXPRESSION": '"arealtype" = 21',
        "OUTPUT": "memory:"
    }
)["OUTPUT"]

# Dissolve → én samlet jordbruksflate
fulldyrka = processing.run(
    "native:dissolve",
    {
        "INPUT": fulldyrka,
...
# Fix fulldyrka for sikkerhets skyld
fulldyrka = processing.run(
    "native:fixgeometries",
    {"INPUT": fulldyrka, "OUTPUT": "memory:"}
)["OUTPUT"]

print("Fulldyrka mark klar.")


# Fjerner overlapp fra natur-lagene
...
for name in layer_names:
    ...
    # Difference → fjern fulldyrka overlapp
    layer = processing.run(
        "native:difference",
        {
            "INPUT": layer,
...
print("Fulldyrka mark er fjernet fra alle naturverdikartlag.")


################################################################################

### Her behandles kartlag for inngrepsfri natur og sammenslås med kartlag for arter og naturtyper
# 1: Fjerner all inngrepsfri natur utenfor min kommune
# 2: Verdikategoriserer kartlaget for inngrepsfri natur og legger til tidligere kartlag (Arter, naturtyper)
# vsone = v kategoriseres som natur av svært stor verdi
# vsone = 1 kategoriseres som natur av stor verdi
# vsone = 2 kategoriseres som natur av moderat verdi 
...
inngrepsfri = project.mapLayersByName("Inngrepsfri natur")[0]
kommune = project.mapLayersByName("Min kommune")[0]
...
clipped.setName("Inngrepsfri natur")

project.removeMapLayer(inngrepsfri.id())
project.addMapLayer(clipped)

print("Inngrepsfri natur er nå begrenset til Min kommune.")
...
svært_stor = project.mapLayersByName("Arter og naturtyper av svært stor verdi")[0]
stor = project.mapLayersByName("Arter og naturtyper av stor verdi")[0]
middels = project.mapLayersByName("Arter og naturtyper av middels verdi")[0]

# --------------------------------------------------

def merge_vsone(vsone_value, target_layer, output_name):

    filtered = processing.run(
        "native:extractbyexpression",
        {
            'INPUT': inngrepsfri,
            'EXPRESSION': f'"vsone" = \'{vsone_value}\'',
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']
    ...
    merged = processing.run(
        ...
        {
            'LAYERS': [target_layer, filtered],
            'CRS': target_layer.crs(),
            'OUTPUT': 'memory:'
        }
    )['OUTPUT']

    merged.setName(output_name)
    project.addMapLayer(merged)

    print(f"{output_name} opprettet.")


# --------------------------------------------------

merge_vsone("v", svært_stor, "Natur_svært_stor")
merge_vsone("1", stor, "Natur_stor")
merge_vsone("2", middels, "Natur_middels")

print("Inngrepsfri natur er nå innarbeidet i verdilagene.")


################################################################################

### Her behandles kartlag for Naturvernområder og foreslåtte verneområder, og 
# sammenslås med kartlag for arter og naturtyper av høyeste verd. Det resulterende
# kartlaget får navn Natur_vern. 

# 1: Slår sammen og dissolve eksisterende og foreslåtte naturvernområder. Disse
# betraktes dermed som det samme 
# 2: Sletter naturvernområder utenfor min kommune
# 3: Slår sammen med eksisterende kartlag: Natur av høyeste verdi (=Vernet natur)  

################################################################################


#🟩# 1: Slår sammen og dissolve eksisterende og foreslåtte naturvernområder


foreslatt = project.mapLayersByName("Foreslått naturvernområde")[0]
vern = project.mapLayersByName("Naturvernområde")[0]
kommune = project.mapLayersByName("Min kommune")[0]
hoy_verdi = project.mapLayersByName("Arter og naturtyper av høyeste verdi")[0]

# Merge
merged_vern = processing.run(
    "native:mergevectorlayers",
    {
        'LAYERS': [foreslatt, vern],
        'CRS': foreslatt.crs(),
        'OUTPUT': 'memory:'
    }
)['OUTPUT']
        ...
        'FIELDS_MAPPING': [
            {
                ...
                'length': 100,
                'precision': 0,
                'expression': "'Eksisterende og foreslåtte naturvernområder'"
            }
        ],
        'OUTPUT': 'memory:'
    }
...
#🟩# 3: Slår sammen med eksisterende kartlag: Natur av høyeste verdi


natur_vern = processing.run(
    "native:mergevectorlayers",
    {
        'LAYERS': [clipped_vern, hoy_verdi],
        'CRS': foreslatt.crs(),
        'OUTPUT': 'memory:'
    }
)['OUTPUT']

natur_vern.setName("Natur_vern")
project.addMapLayer(natur_vern)

print("Eksisterende og foreslåtte naturvernområder i din kommune er slått sammen med arter og naturtyper av høyeste verdi (Utvalgte naturtyper, prioriterte arter, freda arter)")


################################################################################

                    # PYNTER OG GJØR PRESENTABELT ^_^
# Gir endelig navn på de seks kartlagene
# Sletter alle midlertidige kartlag
# Gir kartlagene riktig farge
...
rename_order = [
    ("Arter og naturtyper av potensiell verdi", "Natur av potensiell verdi"),
    ("Arter og naturtyper av noe verdi", "Natur av noe verdi"),
    ("Natur_middels", "Natur av middels verdi"),
    ("Natur_stor", "Natur av stor verdi"),
    ("Natur_svært_stor", "Natur av svært stor verdi"),
    ("Natur_vern", "Vernet natur")
]

for old_name, new_name in rename_order:

    layers = project.mapLayersByName(old_name)
    if not layers:
        print(f"Fant ikke {old_name}")
        continue
    ...
    # Flytt laget til topp i sin gruppe
    clone = node.clone()
...
print("Ferdig med endring av navn til endelig navn.")
...
layers_to_keep = [
    "Vernet natur",
    "Natur av svært stor verdi",
    "Natur av stor verdi",
    "Natur av middels verdi",
    "Natur av noe verdi",
    "Natur av potensiell verdi"
]
...
### Pynter: Gir layers riktige farger 


# Definer lag + farge
layer_styles = {
    "Vernet natur": "#8B0000",   # Veldig mørkerød
    "Natur av svært stor verdi": "#E32C07",# Mørkerød
    "Natur av stor verdi": "#FFA500",      # Oransje
    "Natur av middels verdi": "#FFD700",  # Gul
    "Natur av noe verdi": "#228B22",       # Grønn
    "Natur av potensiell verdi": "#808080" # Grå
}
